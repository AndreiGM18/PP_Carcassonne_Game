...
% altă piesă.
tile(1, T) :- T = ([c, c, p, c], false).
tile(2, T) :- T = ([c, c, d, c], false).
tile(3, T) :- T = ([c, c, p, p], false).
tile(4, T) :- T = ([c, c, p, p], true).
tile(5, T) :- T = ([c, p, c, p], true).
tile(6, T) :- T = ([c, p, c, p], false).
tile(7, T) :- T = ([c, p, p, p], false).
tile(8, T) :- T = ([c, c, d, d], false).
tile(9, T) :- T = ([c, p, d, d], false).
tile(10, T) :- T = ([c, d, d, p], false).
tile(11, T) :- T = ([c, d, p, d], false).
tile(12, T) :- T = ([c, d, d, d], false).
tile(13, T) :- T = ([p, p, d, d], false).
tile(14, T) :- T = ([p, d, p, d], false).
tile(15, T) :- T = ([p, d, d, d], false).
tile(16, T) :- T = ([d, d, d, d], false).
...
at(Tile, Direction, What) :-
	...
	nth0(Index, Directions, Direction),
	nth0(Index, Represent, Elem),
	What = Elem.
...
ccw(Tile, Rotation, RotatedTile) :-
	Tile = (Represent, Has),
	append(Right, Left, Represent),
	length(Right, Rotation),
	append(Left, Right, RotatedRepresent),
	RotatedTile = (RotatedRepresent, Has). 
...
rotations(Tile, RotationPairs) :-
	rotations_helper(Tile, 1, [(0, Tile)], RotationPairs).

rotations_helper(_, 4, RotationPairs, RotationPairs).
rotations_helper(Tile, Rotation, AccRotationPairs, RotationPairs) :-
	ccw(Tile, Rotation, RotatedTile),
	hasRotation((_, RotatedTile), AccRotationPairs),
	!,
	NextRotation is Rotation + 1,
	rotations_helper(Tile, NextRotation, AccRotationPairs, RotationPairs).
rotations_helper(Tile, Rotation, AccRotationPairs, RotationPairs) :-
	ccw(Tile, Rotation, RotatedTile),
	append(AccRotationPairs, [(Rotation, RotatedTile)], NewRotationPairs),
	NextRotation is Rotation + 1,
	rotations_helper(Tile, NextRotation, NewRotationPairs, RotationPairs).

hasRotation((_, RotatedTile), RotationPairs) :-
	member((_, RotatedTileRand), RotationPairs),
	RotatedTile == RotatedTileRand.
...
match(Tile, NeighborTile, NeighborDirection) :-
	at(Tile, NeighborDirection, ElemTile),
	opposite(NeighborDirection, OppositeDirection),
	at(NeighborTile, OppositeDirection, ElemNeigh),
	ElemTile == ElemNeigh.
...
findRotation(Tile, Neighbors, Rotation) :- 
	rotations(Tile, RotationPairs),
	findRotation_helper(RotationPairs, Neighbors, Rotations),
	member(Rotation, Rotations).

findRotation_helper([], _, []).
findRotation_helper([(Rot, RotatedTile) | Tail], Neighbors, [Rot | Rest]) :-
	checkRotation(RotatedTile, Neighbors),
	!,
	findRotation_helper(Tail, Neighbors, Rest).
findRotation_helper([_ | Tail], Neighbors, Rotations) :-
	findRotation_helper(Tail, Neighbors, Rotations).

checkRotation(_, []).
checkRotation(RotatedTile, [(NeighborTile, NeighborDirection) | Tail]) :-
	match(RotatedTile, NeighborTile, NeighborDirection),
	checkRotation(RotatedTile, Tail).
...
canPlaceTile(Board, Pos, Tile) :-
	\+ member((_, Pos), Board),
	findall((NeighborTile, Direction),
			...
			Neighbors),
	Neighbors \= [],
	forall(member((NeighborTile, Direction), Neighbors),
			match(Tile, NeighborTile, Direction)).
...
getAvailablePositions(Board, Positions) :-
	...
	findall((XN, YN),
			...
			between(YMin, YMax, Y),
			member((_, (X, Y)), Board),
			neighbor((X, Y), _, (XN, YN)),
			\+ member((_, (XN, YN)), Board)),
			Duplicate_Positions),
	sort(Duplicate_Positions, Positions).
...
findPositionForTile(Board, Tile, Position, Rotation) :-
	...
	findall((NeighborTile , Direction),
			...
			directions(Directions),
			member(Direction, Directions),
			neighbor(Position, Directions, NeighborPos)),
			Neighbors),
	findRotation(Tile, Neighbors, Rotation),
	ccw(Tile, Rotation, RotatedTile),
